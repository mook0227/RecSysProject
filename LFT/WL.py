...
class WL(nn.Module):
    def __init__(self, args, train_df, train_like, test_like, vali_like, device, MS):
        ...
        self.weight = -MS
        self.weight = self.weight - np.min(self.weight)
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.epoch + 1):
            ...
            for i in tqdm(range(num_batch)):
                ...
                batch_loss = self.train_model_per_batch(batch_matrix, batch_idx)
            ...
            if epoch % self.display == 0:
                ...
                if np.mean(ndcg) > best_result:
                    ...
                    torch.save(self.state_dict(), './Data/' + self.data + '/WL.model')
                    with open('./Data/' + self.data + '/Rec_WL.npy', "wb") as f:
    ...
    def train_model_per_batch(self, batch_matrix, batch_index):
        # zero grad
        ...
        # loss
        weight = torch.FloatTensor(self.weight[batch_index]).to(self.device)
        ce_loss = -((F.log_softmax(output, 1) * batch_matrix).sum(1) * weight).sum() / weight.sum()
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='WL')
    ...
    parser.add_argument('--data', type=str, default='ML20M', help='path to eval in the downloaded folder')
    ...
    MS = np.load('./Data/' + args.data + '/MS_similarity.npy')

    print('!' * 100)

    model = WL(args, train_df, train_like, test_like, vali_like, device, MS)
    model.train_model()
